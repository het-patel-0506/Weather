export default function ForecastStrip({ days = [] }) {
  ...
  return (
    <div className="rounded-2xl shadow-md bg-white dark:bg-slate-800 p-4 lg:p-6">
      <div className="overflow-x-auto flex gap-3 lg:grid lg:grid-cols-3">
        {days.map((d, i) => (
          <button
            key={i}
            type="button"
            className="min-w-[120px] rounded-xl border border-slate-200 dark:border-slate-700 p-3 text-left focus:outline-none focus-visible:ring transition hover:shadow-md"
